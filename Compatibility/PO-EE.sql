...
-- Main Compatibility Code
	...
	UPDATE Language_en_US SET Text = '+15%[ICON_STRENGTH]CS.' WHERE Tag = 'TXT_KEY_PROMOTION_EE_DRYDOCK_HELP' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='PO-EE' AND Value= 1);
	...
	UPDATE Language_en_US SET Text = '+1[ICON_MOVES]Movement Point. +2[ICON_VIEW_CITY]Sight if [COLOR:255:230:85:255]Embarked[ENDCOLOR].' WHERE Tag = 'TXT_KEY_PROMOTION_EE_ADVENTURER_HELP' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='PO-EE' AND Value= 1);
